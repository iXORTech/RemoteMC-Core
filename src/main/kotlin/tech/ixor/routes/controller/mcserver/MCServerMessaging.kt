...
fun Route.mcServerMessaging() {
    ...
    post("/mcserver/say") {
        ...
        if (minecraftServer != null) {
            ...
            if (source == null || sender == null || message == null) {
                call.respondText(I18N.source_sender_or_message_is_null(), status = HttpStatusCode.BadRequest)
                return@post
            }

            val mcServerResponse = minecraftServer.say(source, sender, message)
            when (mcServerResponse.statusCode) {
                200 -> call.respondText(I18N.message_sent(), status = HttpStatusCode.OK)
    ...
    post("/mcserver/broadcast") {
        ...
        if (minecraftServer != null) {
            ...
            if (message == null) {
                call.respondText(I18N.message_is_null(), status = HttpStatusCode.BadRequest)
                return@post
            }

            val mcServerResponse = minecraftServer.broadcast(message)
            when (mcServerResponse.statusCode) {
                200 -> call.respondText(I18N.broadcast_sent(), status = HttpStatusCode.OK)
                401 -> call.respondText(I18N.core_authkey_invalid(), status = HttpStatusCode.Unauthorized)
                503 -> call.respondText(I18N.mcserver_offline(), status = HttpStatusCode.ServiceUnavailable)
                else -> call.respondText(
                    I18N.unknown_error(mcServerResponse.statusCode, mcServerResponse.message),
